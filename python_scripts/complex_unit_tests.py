...
def write_to_cpp(mat,mat_name):
	...
	for i in xrange(2):
		for j in xrange(2):
			string += "%s.setComplex(complex(%g,%g),%d);" % (mat_name,np.real(mat[i,j]),np.imag(mat[i,j]),int(6*i + 2*j))
...
def write_matrices_contigious_complex_su3():
	...
	print "// Adding"
	print write_to_cpp(s1+s2,"sAdd")

	print "// Subtracting"
	print write_to_cpp(s1-s2,"sSub")

	print "// Multiplying"
	print write_to_cpp(s1*s2,"sMul")

	print "// Conjugate of s1"
	print write_to_cpp(s1.conjugate(),"sC")

	print "// Transpose of s1"
	print write_to_cpp(s1.T,"sT")

	print "// Complex conjugate of s1"
	print write_to_cpp(s1.H,"sCT")
