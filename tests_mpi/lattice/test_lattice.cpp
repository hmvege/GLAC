...
bool testLattice()
{
  ...
  // std::unique_ptr<Lattice<SU3>> L = std::unique_ptr{new Lattice<SU3>[4]};
  // std::unique_ptr<Lattice<SU3>> LNew = std::unique_ptr{new Lattice<SU3>[4]};
  for (int i = 0; i < 4; i++)
  {
    L[i].allocate(n_dim);
...
TEST_CASE("Lattice shift test", TAGS + "[shift]") {}
