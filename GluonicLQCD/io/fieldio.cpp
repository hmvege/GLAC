...
void IO::FieldIO::writeFieldToFile(Links * lattice, int configNumber)
{
     ...
     */
    MPI_File file;
    std::string filename = Parameters::pwd + Parameters::outputFolder + Parameters::filename
                                            + "_beta" + std::to_string(Parameters::beta)
                                            + "_spatial" + std::to_string(Parameters::NSpatial)
                                            + "_temporal" + std::to_string(Parameters::NTemporal)
...
void IO::FieldIO::loadFieldConfiguration(std::string filename, Links *lattice)
{
     ...
     */
    MPI_File file;
    MPI_File_open(MPI_COMM_SELF, (Parameters::pwd + Parameters::outputFolder + filename).c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &file);
    MPI_Offset nt = 0, nz = 0, ny = 0, nx = 0;
...
//                    }
    ...
    MPI_File_close(&file);
    if (Parallel::Communicator::getProcessRank() == 0) cout << "Configuration " << Parameters::outputFolder + Parameters::filename << " loaded." << endl;
}
